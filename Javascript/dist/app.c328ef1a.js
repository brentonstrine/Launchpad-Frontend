...
const getRemoteComments = async () => {
  try {
    const res = await _axios.default.get(`${BASEL_URL}`);
...
const formEvent = form.addEventListener('submit', async event => {
  event.preventDefault(); // Handle the time a user posts a message by using the current time
...
const addComment = async comment => {
  try {
    const res = await _axios.default.post(`${BASEL_URL}`, comment);
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "65293" + '/');
